...
Use `--batch-size 0` when you want to upload every pending image up front. A
nonzero batch size now streams image chunks into one live Colab runtime: the
remote worker encodes prompts, loads LTX once, generates the current chunk, then
waits for the local runner to upload the next chunk. Set
`LTX_COLAB_STREAM_BATCHES=0` only if you intentionally want the old behavior
where each batch exits and the next attempt reloads the model.
...
## Quality-First Short Picker V3

V3 is additive and leaves the original `antigravity-clipper` implementation and
its outputs unchanged. It finds compact hook/chorus/drop nuclei, expands them to
clean musical boundaries, keeps one edit champion per event, and returns results
in strict quality order. By default, it returns only event champions scoring at
least 94% as highly as the song champion. V3 writes only in the `shorts/v3/`
namespace; an optional `--output-subdir` must also remain inside that namespace.

Analyze the latest completed video without exporting clips:

```bash
antigravity-clipper-v3 \
  --outputs-dir outputs \
  --latest-only \
  --no-export
```

Export the quality-ranked recommendations after reviewing
`shorts/v3/selections.json` and `shorts/v3/candidates.json`:

```bash
antigravity-clipper-v3 --outputs-dir outputs --latest-only --force
```

Each export run gets an isolated directory below `shorts/v3/exports/`. The
canonical `selections.json` report is updated only after every requested clip
exports successfully, so a partial export cannot replace the last good result.

Historical `shorts/1This` choices can be evaluated as an optional, strongly
regularized preference signal. V3 enables it only when leave-one-track-out
validation beats its safety threshold; otherwise the report records why it was
disabled and uses structural scoring alone.

```bash
antigravity-clipper-v3 \
  --outputs-dir outputs \
  --latest-only \
  --preference-history outputs \
  --no-export
```

After generating V3 reports for historical runs, inspect event-ranking metrics
without touching any clips:

```bash
python scripts/evaluate_picker_v3.py --outputs-dir outputs
```
...
--batch-size 8           Stream image chunks of 8 into one loaded Colab worker
